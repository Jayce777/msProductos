package com.uisrael.ms_productosMDB.controller;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import com.uisrael.ms_productosMDB.model.TipoProducto;
import com.uisrael.ms_productosMDB.services.ITipoProductoService;

@RestController
@RequestMapping("/api/tipoProductos")
public class TipoProductoController {

	@Autowired
	private ITipoProductoService tiposervice;
	
	@PostMapping
	@ResponseStatus(code = HttpStatus.OK)
	public boolean crearTipo(@RequestBody TipoProducto nuevo) {
		
		return tiposervice.crearTipo(nuevo);
	}
	
	@GetMapping
	@ResponseStatus(code = HttpStatus.OK)
	public List<TipoProducto> obtenerTipos() {
		
		return tiposervice.otenerTipo();
	}

}
